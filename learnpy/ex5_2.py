...
'''
1. Parse the CDP data to obtain the following information:
hostname \ Device ID: R2
ip \ IP address: 10.1.1.2
model \ Platform: Cisco 881,
vendor \ Platform: Cisco 881,
device_type (device_type will be either 'router', 'switch', or 'unknown'). \ Capabilities: Router Switch IGMP
'''
...
# Parse all "show cdp neibhbors detail output"
for cdp_neighbors_detail in show_cdp_neighbors_detail:

  # Split output into lines
  cdp_neighbors_detail = cdp_neighbors_detail.split('\n')

  # Parse each line and extract values
  for line in cdp_neighbors_detail:
    if 'Device ID:' in line:
      (discard, hostname) = line.split(': ')
    elif 'IP address:' in line:
      (discard, ip) = line.split(': ')
    elif 'Platform:' in line:
      (platform, capabilities) = line.split(', ')
      (vendor, model) = platform.split(': ')[1].split()
      (device_type) = capabilities.split(': ')[1].split()[0]

      # Create device in network_devices dictionary
      network_devices[hostname] = {}
      # Add values to device dictionary
...
# Parse all "show cdp neibhbors output"
for cdp_neighbors in show_cdp_neighbors:

  # Split output
  cdp_neighbors = cdp_neighbors.strip()
  cdp_neighbors = cdp_neighbors.split('\n')

  # Extract hostname
  hostname = cdp_neighbors[0].split('>')[0]

  # Extract adjacent_devices for hostname
  for i,line in enumerate(cdp_neighbors):
    if 'Device ID' in line:
      i += 1
      neighbors = cdp_neighbors[i:]
      adjacent_devices = []
      for neighbor in neighbors:
        neighbor = neighbor.split()[0]
        adjacent_devices.append(neighbor)
        network_devices[hostname]['adjacent_devices'] = adjacent_devices
...
'''
  for line in show_cdp_neighbors_detail:
    words = line.split()
    for i, word in enumerate(words):
      if word in ('Device', 'IP', 'Platform:'):
        i += 2
        print words[i]
